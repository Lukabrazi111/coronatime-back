<form action="{{ route('register.store') }}" method="post" class="flex flex-col w-full mt-4 md:w-6/12">
    @csrf
    ...
    @else
        ...
        <div class="flex flex-col relative">
            ...
            <input
                class="px-4 py-4 rounded-lg border @if ($email) @error('email') border-red-600 @else border-green-600 @enderror @else border-gray-200 @endif mb-2 placeholder-dark"
        ...
        <div class="flex flex-col relative">
            <label for="repeatPassword" class="mb-2">{{ __('Repeat password') }}</label>
            <input id="repeatPassword"
                   class="px-4 py-4 rounded-lg border border-gray-200 mb-2 placeholder-dark" type="password"
                   name="repeatPassword" placeholder="{{ __('Repeat password') }}">
